...
def calculate(ML: bool, PRUNE: bool, SPEED: str, DWT_MULTIPLIER: float, SPEED_MULTIPLIER: float,LOAD_PORT_DELAY: float, DISCHARGE_PORT_DELAY: float, VLSF_BUFFER_PCT: float, MGO_BUFFER_PCT: float,BUNKER_PORT_COST: float) -> None:
    ...
    if ML:
        # Save all pairwise results (ML)
        df.to_csv("ML_cases.csv", index=False)
    else:
        # Save all pairwise results (base case)
    ...
    # Save fleet plan
    chosen.to_csv("fleet_assignment_ml.csv" if ML else "fleet_assignment_base.csv", index=False)
...
if __name__ == "__main__":
    ...
    # define constants
    ML = False                  # True / False              --> for enabling / disabling ml on base case
    ...
    print("=" * 60)
    calculate(ML, PRUNE, SPEED, DWT_MULTIPLIER, SPEED_MULTIPLIER, LOAD_PORT_DELAY, DISCHARGE_PORT_DELAY, VLSF_BUFFER_PCT, MGO_BUFFER_PCT, BUNKER_PORT_COST)

    print("\n\n\n")
    print("=" * 60)
    print("Calculating ML case:")
    print("=" * 60)
    calculate(True, PRUNE, SPEED, DWT_MULTIPLIER, SPEED_MULTIPLIER, LOAD_PORT_DELAY, DISCHARGE_PORT_DELAY, VLSF_BUFFER_PCT, MGO_BUFFER_PCT, BUNKER_PORT_COST)
